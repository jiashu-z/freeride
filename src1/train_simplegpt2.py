...
import argparse
import torch
from bubblebandit.scheduler_v1 import SchedulerClient
...
def load_zero_bubble_stats() -> dict[tuple[str, int], dict[tuple[int, str], float]]:
    configs: list[str] = [
        "large",
        "xlarge",
        "xxlarge",
    ...
    batch_nums: list[int] = [4, 5, 6, 7, 8]
    stage_ids: list[int] = [0, 1, 2, 3]
    bubble_types: list[str] = ["A", "B", "C", "D"]
    config_batch_num_bubble_stat_dict: dict[
        tuple[str, int], dict[tuple[int, str], float]
    ] = {}
    for config, batch_num in product(configs, batch_nums):
        bubble_stat_dict: dict[tuple[int, str], float] = {}
        for stage_id, bubble_type in product(stage_ids, bubble_types):
            bubble_stat_dict[(stage_id, bubble_type)] = 0
        config_batch_num_bubble_stat_dict[(config, batch_num)] = bubble_stat_dict
    return config_batch_num_bubble_stat_dict


def load_bubble_stats() -> dict[tuple[str, int], dict[tuple[int, str], float]]:
    configs: list[str] = [
        # "large",
        "xlarge",
        # "xxlarge",
    ...
    for config, batch_num in product(configs, batch_nums):
        bubble_stat_df = pd.read_csv(
            os.path.join(f"./log/e2e_vanilla_deepspeed_baseline_ada6000_{config}_10_{batch_num}", f"bubble_summary_{batch_num}.csv")
        )
        bubble_stat_dict: dict[tuple[int, str], float] = {}
        for stage_id, bubble_type in product(stage_ids, bubble_types):
            values = bubble_stat_df[
                (bubble_stat_df["GPU"] == stage_id)
                & (bubble_stat_df["Type"] == bubble_type)
            ]["Duration (sec)"]
            if len(values) == 0:
                continue
            bubble_stat_dict[(stage_id, bubble_type)] = values.min()
        config_batch_num_bubble_stat_dict[(config, batch_num)] = bubble_stat_dict
    return config_batch_num_bubble_stat_dict
...
if __name__ == "__main__":
    ...
    config_batch_num_bubble_stat_dict = load_bubble_stats()
    # zero_bubble_stat_dict = load_zero_bubble_stats()
    print(config_batch_num_bubble_stat_dict)
    engine.stage_bubble_durations = config_batch_num_bubble_stat_dict[
        (args.config.lower(), args.batch_num)
    ]
    print(engine.stage_bubble_durations)

    engine.scheduler_client = SchedulerClient("localhost:40052")
    engine.logger_client = LoggerClient(args.logger_addr)
    # engine.counter = 0
    ...
    if stage_id in [0, 1, 2, 3]:
        ...
        with open(f"{name}_stage{stage_id}.json", "w") as fout:
