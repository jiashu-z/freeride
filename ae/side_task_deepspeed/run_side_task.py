...
for workload_meta, config in product(workloads, configs):
    ...
    command_prefix: str = (
        f"python3 -m bubblebandit.add_task -n {name} -s localhost:40052"
    )
    gpu_memory = int(
        math.ceil(
    ...
    task_num: int = len(
        list(
            filter(
                lambda x: x > gpu_memory, [v for v in stage_id_gpu_memory_map.values()]
            )
        )
    )
    print("GPU Memory:", gpu_memory, "Task Num:", task_num)
    ...
    while True:
        ...
        if all([t <= 50 for t in temperatures]):
            break
